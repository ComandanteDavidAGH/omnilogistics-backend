"""Exportador de Auditorías Económicas de Genesis Core v1.3.

Genera libros de Excel con diseño ejecutivo gerencial C-Level:
  - Sincronizado con la Taxonomía Financiera Estricta (Observada vs. Estimada).
...
COLOR_HEADER_BG = "0F172A"       # Azul Marino Oscuro / Navy (Banner Superior)
...
def _apply_corporate_table(ws, start_row: int, df: pd.DataFrame, currency_cols=None, pct_cols=None):
    ...
    for row_idx, row_data in enumerate(df.values, start_row + 1):
        ...
        for col_idx, val in enumerate(row_data, 1):
            ...
            if col_name in currency_cols or "COP" in col_name or "Monto" in col_name or "Impacto" in col_name:
                if isinstance(val, (int, float)):
                    cell.number_format = '"$"#,##0'
                cell.alignment = Alignment(horizontal="right", vertical="center")
            elif col_name in pct_cols or "Margen" in col_name or "%" in col_name:
                if isinstance(val, (int, float)):
    ...
    # Desactivar líneas de cuadrícula para acabado ejecutivo
...
def build_audit_workbook(audit: dict, findings: list, tasks: list) -> bytes:
    ...
    _add_sheet_banner(ws_resumen, "GENESIS CORE v1.3 — INFORME DE AUDITORÍA ECONÓMICA", sub_info, max_col=8)
    ...
    kpis = [
        ...
        ("PÉRDIDA DIRECTA", (fin.get("dineroEnRiesgoDetalle") or {}).get("perdida_directa_observada", 0), '"$"#,##0'),
    ]
    ...
    # Tabla Desagregada de Indicadores y Taxonomía Financiera
    det = fin.get("dineroEnRiesgoDetalle") or {}
    resumen_data = [
        ...
        ["--- TAXONOMÍA FINANCIERA ---", "--- MONTO DETECTADO (COP) ---"],
        ["Pérdida Directa Observada (Confirmada)", det.get("perdida_directa_observada", 0)],
        ["Sobrecosto Potencial (Duplicados)", det.get("sobrecosto_potencial", 0)],
        ["Diferencia por Reconciliar (Multi-fuente)", det.get("diferencia_por_reconciliar", 0)],
        ["Desviación Estadística Estimada", det.get("desviacion_estadistica_estimada", 0)],
        ["EXPOSICIÓN ECONÓMICA TOTAL", fin.get("dineroEnRiesgo", 0)],
    ]
    df_resumen = pd.DataFrame(resumen_data, columns=["Métrica / Categoría Financiera", "Monto / Valor Evaluado"])
    _apply_corporate_table(ws_resumen, start_row=8, df=df_resumen, currency_cols=["Monto / Valor Evaluado"])
    ...
    for f in findings:
        ...
        hallazgos_rows.append({
            ...
            "Categoría": f.get("tipo"),
            "Título del Hallazgo": f.get("titulo"),
            "Impacto Directo (COP)": imp.get("impacto_directo"),
            "Descripción de Impacto": imp.get("descripcion"),
    ...
    _apply_corporate_table(ws_hallazgos, start_row=4, df=df_hallazgos, currency_cols=["Impacto Directo (COP)"])
